...
minetest.register_abm{
     	nodenames = {"ecobots:ecobots_pioneer_shrub_bot"},
	...
	--count bots
		...
		local ps, cn = minetest.find_nodes_in_area(
			...
			{x = pos.x + poprad, y = pos.y + poprad, z = pos.z + poprad}, {"ecobots:ecobots_pioneer_shrub_bot"})
		num_bot = (cn["ecobots:ecobots_pioneer_shrub_bot"] or 0)
	...
	--count wood
		...
		local ps, cn = minetest.find_nodes_in_area(
			...
			{x = pos.x + wood_poprad, y = pos.y + wood_poprad, z = pos.z + wood_poprad}, {"ecobots:ecobots_pioneer_shrub_trunk"})
		num_wood = (cn["ecobots:ecobots_pioneer_shrub_trunk"] or 0)
...
--note, the trunk limit seems not to work... don't know why
	 local randpos_trunkbelow = {x = pos.x, y = pos.y - 2, z = pos.z}
...
-- Create new bot if location has an older bot or wood below
	...
	if minetest.get_node(randpos_below).name == "ecobots:ecobots_pioneer_shrub_bot" or minetest.get_node(randpos_below).name == "ecobots:ecobots_pioneer_shrub_trunk" then
	


---check if air above and trunk not too long 

	if minetest.get_node(randpos_above).name == "air" and randpos_trunkbelow ~= "ecobots:ecobots_pioneer_shrub_trunk" then
...
--create bot and wood
			...
			minetest.set_node(randpos_above, {name = "ecobots:ecobots_pioneer_shrub_bot"})
			minetest.set_node(randpos, {name = "ecobots:ecobots_pioneer_shrub_trunk"})
			minetest.set_node(randpos_below, {name = "ecobots:ecobots_pioneer_shrub_trunk"})
...
minetest.register_abm{
	...
	---get time
		...
		if minetest.get_node(pos_above).name == "ecobots:ecobots_pioneer_shrub_bot" or minetest.get_node(pos_above).name == "ecobots:ecobots_pioneer_shrub_trunk" then	

		-- turn to wood
			minetest.set_node(pos, {name = "ecobots:ecobots_pioneer_shrub_trunk"})
...
minetest.register_abm{
     	nodenames = {"ecobots:ecobots_pioneer_shrub_trunk"},
	interval = 1,
	chance = 5,
	catch_up = false,
	action = function(pos)
...
--create bot 
		...
		minetest.set_node(randpos, {name = "ecobots:ecobots_pioneer_shrub_bot"})
