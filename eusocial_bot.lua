...
--- do if bot finds grass in nearby node
	...
	if minetest.registered_nodes[minetest.get_node(randpos).name].groups.flora == 1 then
...
minetest.register_abm{
	...
	if minetest.registered_nodes[minetest.get_node(pos_below).name].groups.soil == 1 then
...
minetest.register_abm{
	...
	--count food
		...
		local ps = minetest.find_nodes_in_area(
			{x = pos.x - poprad, y = pos.y - poprad, z = pos.z - poprad},
			{x = pos.x + poprad, y = pos.y + poprad, z = pos.z + poprad}, {"group:flora"}, true)
		local cn = 0
		if ps ~= nil then
			for _ in pairs(ps) do
				cn = cn + 1
			end
		end
		num_food = (cn or 0)
...
--count dirt
		...
		local ps, cn = minetest.find_nodes_in_area(
			{x = pos.x - poprad, y = pos.y - poprad, z = pos.z - poprad},
			{x = pos.x + poprad, y = pos.y + poprad, z = pos.z + poprad}, {"group:soil"}, true)
...
--count fed nests for pop limits
		...
		local ps = minetest.find_nodes_in_area(
			...
			{x = pos.x + poprad_nest, y = pos.y + poprad_nest, z = pos.z + poprad_nest}, {"ecobots:ecobots_eusocial_nest_fed"}, true)
		local cn = 0
		if ps ~= nil then
			for _ in pairs(ps) do
				cn = cn + 1
...
-- do if the new site is dirt
	...
	if minetest.registered_nodes[minetest.get_node(randpos_newnest).name].groups.soil == 1 then
...
if minetest.registered_nodes[minetest.get_node(pos_below).name].groups.soil == 1 or minetest.get_node(pos_below).name == "ecobots:ecobots_eusocial_trail" then
...
if minetest.registered_nodes[minetest.get_node(randpos_dig).name].groups.soil == 1 then
