...
----------------------------------------------------------------
--GENERALIST TREE BOT
-- A balanced tree between the fast growth of the pioneers and the shade tolerance of swamp forest
--can adopt a wider variety of forms. Often starts shrublike so no need for a generalist shrub. Forms a beautiful broad canopy.
----------------------------------------------------------------
--SETTINGS
...
local tree_rootgrowth = tree_leafygrowth * 2
...
minetest.register_abm{
	...
	--count bots
		...
		local ps, cn = minetest.find_nodes_in_area(
			...
			{x = pos.x + poprad, y = pos.y + poprad, z = pos.z + poprad}, {"ecobots:ecobots_generalist_tree_bot"})
		num_bot = (cn["ecobots:ecobots_generalist_tree_bot"] or 0)
...
-- Create new bot if location has an older bot or wood below
	...
	if minetest.get_node(randpos_below).name == "ecobots:ecobots_generalist_tree_bot" or minetest.get_node(randpos_below).name == "default:tree" then
	


---check if air above and trunk not too long 

	if minetest.get_node(randpos_above).name == "air" and randpos_trunkbelow ~= "default:tree" then
...
--create bot and wood
			...
			minetest.set_node(randpos_above, {name = "ecobots:ecobots_generalist_tree_bot"})
...
--- above empty space 2 blocks and grounded side growth
	...
	minetest.get_node(randpos_ranside).name == "default:tree" then
			...
			minetest.set_node(randpos, {name = "ecobots:ecobots_generalist_tree_bot"})
...
---high level so shading becomes a factor
	if  light_level >= 15 and (num_bot) < poplim then
...
--- side is attached to leaves or trunk
	if minetest.get_node(randpos_ranside).name == "ecobots:ecobots_generalist_tree_bot" or minetest.get_node(randpos_ranside).name == "default:tree" then
...
--check if hovering
	...
	-- turn to wood if in a pile
		if minetest.get_node(pos_above).name == "ecobots:ecobots_generalist_tree_bot" or minetest.get_node(pos_above).name == "default:tree" then	
